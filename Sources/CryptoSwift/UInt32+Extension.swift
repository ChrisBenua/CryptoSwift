...
extension UInt32 {
  ...
  init<T: Collection>(bytes: T, fromIndex index: T.Index) where T.Element == UInt8, T.Index == Int {
    ...
    let val0 = count > 0 ? UInt32(truncatingIfNeeded: bytes[index]) << 24 : 0
    let val1 = count > 1 ? UInt32(truncatingIfNeeded: bytes[index + 1]) << 16 : 0
    let val2 = count > 2 ? UInt32(truncatingIfNeeded: bytes[index + 2 ]) << 8 : 0
    let val3 = count > 3 ? UInt32(truncatingIfNeeded: bytes[index + 3]) : 0
